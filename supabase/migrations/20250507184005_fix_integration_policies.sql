-- Verificar se a função existe antes de removê-la
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM pg_proc 
        WHERE proname = 'setup_integration_policies' 
        AND pronamespace = 'public'::regnamespace
    ) THEN
        DROP FUNCTION public.setup_integration_policies;
    END IF;
END $$;

-- Remover a função antiga se existir
DROP FUNCTION IF EXISTS public.setup_integration_policies(UUID);
